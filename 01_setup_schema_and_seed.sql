...
CREATE OR REPLACE FUNCTION borrow_book(p_patron_id INT, p_book_id INT)
RETURNS VOID AS $$
BEGIN
  UPDATE books SET available = FALSE WHERE id = p_book_id;
  UPDATE patrons SET borrowed_books = borrowed_books || p_book_id WHERE id = p_patron_id;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION return_book(p_patron_id INT, p_book_id INT)
RETURNS VOID AS $$
BEGIN
  UPDATE books SET available = TRUE WHERE id = p_book_id;
  UPDATE patrons SET borrowed_books = array_remove(borrowed_books, p_book_id) WHERE id = p_patron_id;
END;
$$ LANGUAGE plpgsql;
